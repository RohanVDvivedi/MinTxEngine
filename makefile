...
PUBLIC_HEADERS:=mini_transaction_engine.h mini_transaction.h abort_errors_list.h mini_transaction_engine_stats.h log_record.h mini_transaction_engine_wale_only_functions.h mini_transaction_engine_bufferpool_only_functions.h mini_transaction_engine_page_alloc.h mini_transaction_engine_allotment.h dirty_page_table_entry.h page_allocation_hints.h zlib_zstream_cache.h
...
LFLAGS:=-L${LIB_DIR} -l${PROJECT_NAME} -ltuplestore -lwale -lz -lbufferpool -lblockio -lboompar -llockking -lcutlery -lpthread
# Archiver
AR:=ar rcs
