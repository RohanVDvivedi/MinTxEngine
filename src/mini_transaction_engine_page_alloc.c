...
int free_page_for_mini_tx(mini_transaction_engine* mte, mini_transaction* mt, uint64_t page_id, int* abort_error)
{
	pthread_mutex_lock(&(mte->global_lock));
		...
		pthread_mutex_unlock(&(mte->global_lock));
		int result = free_write_latched_page_INTERNAL(mte, mt, page_to_free, page_id, abort_error);
		pthread_mutex_lock(&(mte->global_lock));
...
void* get_new_page_with_write_latch_for_mini_tx(mini_transaction_engine* mte, mini_transaction* mt, uint64_t* page_id_returned, int* abort_error)
{
	void* new_page = NULL;
	...
	// allocate a new page firstly by attempting to do it without database expansion
	{
		...
		new_page = allocate_page_from_hints_without_database_expansion_INTERNAL(mte, mt, page_id_returned, abort_error);
	...
	// allocate a new page secondly by attempting to do it with database expansion
	{
		...
		// this function needs to be called with shared_lock on manager_lock
		new_page = allocate_page_with_database_expansion_INTERNAL(mte, mt, page_id_returned, abort_error);
