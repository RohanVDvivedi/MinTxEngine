...
// below three are _INTERNAL functions to be called without global_lock held
...
void* allocate_page_from_hints_without_database_expansion_INTERNAL(mini_transaction_engine* mte, mini_transaction* mt, uint64_t* page_id, int* abort_error); // checks all pages from a few of the page_allocation_hints module, and use the one that is found
void* allocate_page_without_database_expansion_INTERNAL(mini_transaction_engine* mte, mini_transaction* mt, uint64_t* page_id, int* abort_error); // checks all pages until one is found and allocated
