...
/*
	This is a separate module from the MinTxEngine project, which is only concerned with managing hints for page allocation over extents in the actual database file
	The term extent here is used for the first time in this project

	The extent is defined as a single group of pages, consistenint of a single free_space_mapper_page + data_pages of count (= is_valid_bits_count_on_free_space_mapper_page(stats)), number of pages
	...
	Each page in this module is 4096 bytes big, regardless of the page size used in the actual database file

	This module is optional and may not be part of your actual page allocation flow, unless everything works out just well.
	It's pages are hints and not mini-transaction controlled ACID compliant, i.e. they do not get WAL-logged OR even stay in bufferpool for long, they are made to be flushed as quickly as possible
	And they are hints, and not source of truth, they only suggest, if a particular extent has any free pages in it, the actual source of truth of this information is the free space mapper page in the extent itself

	free_space_mapper_page is the first page in any of the extent
*/
...
/*
	page organization for the hints tree
	page_no -> level
	0    4
	1    3
	2    2
	...
	which is equal to the number of L level pages + total number of L-1 level pages + total number of L-2 level pages, ... sum until the total number of 0 level pages
	...
	to find the indices of the extent no E, just write the integer E in base P (= 4096 * 8)
	level 0 = E % P
	level 1 = (E / P) % P
	level 2 = (E / (P^2)) % P
	level 3 = (E / (P^3)) % P
	...
	we will always start traversals, by descending from root page, having page_id = 0, at level 4 (MAX_LEVEL), and (it being at index 0 in its immediate parent's subtree which does not exists)
	and then descend to lower levels (towards level 0), using the indices we found in the previous step
*/
...
// this is equal to smallest integral value for MAX_LEVEL where, 2^64 <= PAGE_ALLOCATION_HINTS_BITS_COUNT_PER_NODE ^ (MAX_LEVEL + 1)
// this will also remain fixed forever!!
#define MAX_LEVEL 4
...
struct page_allocation_hints
{
	...
	// this bufferpool is small about mo more than 25 pages, each as expected to be 4KB in size
	bufferpool bf;

	// presistent memory structures end here
	// and in-memory structures start from here
	...
	// these are writes batched, for the calls to update_hints_in_page_allocation_hints() function calls
	...
	// this is the cached results, for suggest_extents_from_page_allocation_hints() function calls
	bst results_set;
};
