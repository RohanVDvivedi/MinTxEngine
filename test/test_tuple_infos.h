...
positional_accessor KEY_POS[1] = {STATIC_POSITION(0), STATIC_POSITION(2)};
compare_direction CMP_DIR[1] = {ASC, ASC};
...
void num_in_words(char* output, uint16_t n) {
  if (n < 20) {
    ...
    sprintf(output, "%s hundred %s", ones[(n / 100) % 10], number_to_words(n % 100));
  } else {
...
int validate_record(const void* buffer)
{
	...
	uint16_t o = find_order(order);
	{
		char t1[1000];
		num_in_words(t1, o);
		char* t2 = get_value_from_element_from_tuple(record_def, STATIC_POSITION(2), buffer).string_value;
